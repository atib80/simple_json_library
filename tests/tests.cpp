...
TEST (simple_json_library, creating_a_default_json_object)
{
  Json json;
  ASSERT_EQ (
      std::get<std::nullptr_t> (json.get_json_value_as_variant ()),
      nullptr);
}
...
{
  Json json{ nullptr };
  auto json_value = json.get_json_value_as_variant ();
  ASSERT_TRUE (
      std::get<std::nullptr_t> (json_value) == nullptr);
  auto json_element_value = json.get_json_element_as_json_null ("");
  ASSERT_TRUE (json_element_value.has_value ()
               && json_element_value.value () == nullptr);
}

TEST (simple_json_library, creating_a_json_object_with_boolean_value)
{
  Json json1{ true };
  auto json1_value = json1.get_json_value_as_variant ();
  ASSERT_TRUE (std::get<bool> (json1_value));
  auto json_element_value1 = json1.get_json_element_as_json_boolean ("");
  ASSERT_TRUE (json_element_value1.has_value ()
               && json_element_value1.value ());

  Json json2{ false };
  auto json2_value = json2.get_json_value_as_variant ();
  ASSERT_TRUE (!std::get<bool> (json2_value));
  auto json_element_value2 = json2.get_json_element_as_json_boolean ("");
  ASSERT_TRUE (json_element_value2.has_value ()
               && !json_element_value2.value ());
}
...
TEST (simple_json_library, creating_a_json_object_with_number_value)
{
  ...
  Json json{ 5.32 };
  ASSERT_TRUE (
      std::abs (std::get<double> (json.get_json_value_as_variant ())
                - 5.32)
      < epsilon);
  auto json_element_value = json.get_json_element_as_json_number ("");
  ASSERT_TRUE (json_element_value.has_value ()
...
TEST (simple_json_library, creating_a_json_object_with_string)
{
  ...
  ASSERT_EQ (std::get<std::string> (json1.get_json_value_as_variant ()),
             apple);
  auto json_element_value1 = json1.get_json_element_as_json_string ("");
  ASSERT_TRUE (json_element_value1.has_value ()
               && (json_element_value1.value () == apple));

  static constexpr std::string car_key{ "car key" };
  Json json2{ car_key };
  ASSERT_EQ (std::get<std::string> (json2.get_json_value_as_variant ()),
             car_key);
  auto json_element_value2 = json2.get_json_element_as_json_string ("");
  ASSERT_TRUE (json_element_value2.has_value ()
               && (json_element_value2.value () == car_key));
}

TEST (simple_json_library, creating_a_json_object_with_array)
{
  static constexpr double epsilon{ 0.000001 };
  constexpr std::array<Json, 5> array_elements{ Json{1.1}, Json{2.2}, Json{3.3}, Json{4.4}, Json{5.5} };
  ...
  for (size_t i{}; i < json_arr2.size (); ++i)
    {
      ASSERT_TRUE (std::abs (json_arr2[i].to_number () - array_elements[i].to_number())
                   < epsilon);
    }
}
...
TEST (simple_json_library, creating_a_json_object_with_map)
{
  ...
  std::unordered_map<std::string, Json> map_elements{ {"name", Json("Alice")}, {"age", Json(25.)}, {"is_student", Json(true)}, 
  {"scores", Json(std::vector<Json>{Json(88.5), Json(97.), Json(78.)})}, 
  {"address", Json(std::unordered_map<std::string, Json>{{"city", Json("Los Angeles")}, {"zip", Json(90001)}})} };
  ...
  ASSERT_EQ (json1.get_json_value_as_object ()->get ().size (),
             map_elements.size ());
  const std::unordered_map<std::string, Json> &json_map1{ std::get<std::unordered_map<std::string, Json> > (
      json1.get_json_value_as_variant ()) };
  ...
  const std::unordered_map<std::string, Json> &json_map2{ std::get<std::unordered_map<std::string, Json> > (
      json2.get_json_value_as_variant ()) };
...
TEST (simple_json_library, creating_a_json_object_by_parsing_input_string)
{
  ...
  auto json_name_value{ json_object->get_json_element_as_json_string (
      "name") };
  ASSERT_TRUE (json_name_value.has_value ());
  ASSERT_EQ (json_name_value.value (), "Alice");

  auto json_age_value{ json_object->get_json_element_as_json_number ("age") };
  ASSERT_TRUE (json_age_value.has_value ());
  ASSERT_EQ (json_age_value.value (), 25);

  auto json_is_student_value{ json_object->get_json_element_as_json_boolean (
      "is_student") };
  ...
  auto json_scores_array_optional{
    json_object->get_json_element_as_json_array ("scores")
  };
  ASSERT_TRUE (json_scores_array_optional.has_value ());
  const auto &json_scores_array = json_scores_array_optional.value ().get ();
  ASSERT_EQ (json_scores_array.size (), 3);
  for (size_t i{}; i < std::min (json_scores_array.size (), scores.size ());
       ++i)
    {
  ...
  auto json_address_object_optional{
    json_object->get_json_element_as_json_object ("address")
  };
  ...
  ASSERT_TRUE (!json_city_value.empty ());
  ASSERT_EQ (json_city_value, "Los Angeles");
...
TEST (simple_json_library, creating_a_json_object_by_parsing_json_string_literal)
{
